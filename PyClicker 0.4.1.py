...
from AliasGeneralFunctions import shorten_number, time_to_units, manage_variable, link_functions
...
def load(new_game=False):
    ...
    if not new_game:
        ...
        for b in save_file:
            if b in ["cheater", "money", "total_money", "run_start", "last_on", "user_id", "cps_speed", "cpc_bonus", "redeemed_codes", "online", "event_bonus"]:
                player[b] = save_file[b]

        for b in ["buildings", "upgrades", "achievements"]:
...
# noinspection PyTypeChecker
player_button = Button(left, image=image, command=player_button_press)
...
def update_buildings(building=None):
    ...
    if right_view.get() == "b":
        ...
        Button(scrolling_frame, command=view_achievements, text="View achievements").pack(pady=(5, 10))
...
def arg_parser():
    ...
    global update_buildings

    if len(argv) == 1:
        return

    args = argv[1:]
    messagebox.showinfo("Args Detected", f"Warning: Game arguments may\nbe unstable "
                                             f"and could cause issues.\n\nCurrent args: {", ".join(args)}")

    for arg in [b for b in args if len(b.split("=")) == 1]:
        if arg == "neverclick":
            player_button.configure(state="disabled")
            if sum([player["buildings"][b]["owned"] for b in player["buildings"]]) == 0:
                player["buildings"]["1"]["owned"] = 1

        elif arg == "onstrike":
            def on_strike():
                for b in player["buildings"]:
                    player["buildings"][b]["owned"] = 0
                update_ui()

            update_buildings = link_functions(on_strike, update_buildings)

        elif arg == "primitive":
            def on_strike():
                for b in player["buildings"]:
                    player["buildings"][b]["unlocked"] = False
                update_ui()
    ...
    for arg in [b for b in args if len(b.split("=")) > 1]:
        if arg[0] == "money":
            player["cheater"] = True
            update_money(arg[1], True)


main.after_idle(lambda: main.after(1, arg_parser))
